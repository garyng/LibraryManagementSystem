...
{
	/// <summary>
	/// Extensions for async operations where Task appears in the left operand.
	/// </summary>
	public static class AsyncResultExtensionsLeftOperand
	{
		...
		public static async Task<Result<T>> OnSuccess<T>(this Task<Result<T>> resultTask, Func<T> func)
		{
			Result<T> result = await resultTask.ConfigureAwait(false);
			return result.OnSuccess(func);
		}
		...
		public static async Task<Result<T>> OnFailure<T>(this Task<Result<T>> resultTask, Action<Result<T>> action)
		{
			Result<T> result = await resultTask.ConfigureAwait(false);
			return result.OnFailure(action);
		}

		public static async Task<Result> OnFailure(this Task<Result> resultTask, Action<Result> action)
		{
			Result result = await resultTask.ConfigureAwait(false);
			return result.OnFailure(action);
		}
